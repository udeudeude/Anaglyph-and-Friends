...
import ColorRevealBuilder from './ColorRevealBuilder.tsx'
import LayeredCompositeBuilder from './LayeredCompositeBuilder.tsx'
import PrintCalibrationBuilder from './PrintCalibrationBuilder.tsx'
import PrintPageBuilder from './PrintPageBuilder.tsx'
...
import { hostedBrowserDepthEnabled } from './browserDepth.ts'
import type { PrintPageIncomingArtwork } from './printPageAssets.ts'
...
type StudioSurface = 'editor' | 'phantogram' | 'colorreveal' | 'layered' | 'printcalibration' | 'printpage'
...
function App() {
    ...
    const [printPageIncoming, setPrintPageIncoming] = useState<PrintPageIncomingArtwork | null>(null)
    ...
    const visibleStageLabel = hostedEdition && processingStage === 'ready'
        ...
        : hostedEdition && processingStage === 'idle'
            ? 'Browser AI workspace'
            : stageLabels[processingStage]

    return <div className="appShell">
        <header className="topBar"><div><div className="brandEyebrow">{hostedEdition ? 'WEB AI 3D WORKSPACE' : 'LOCAL AI 3D WORKSPACE'}</div><h1 className="brandTitle">Anaglyph &amp; Friends</h1></div><div className="topBarActions"><nav className="workspaceModeNav" aria-label="Workspace"><button className={workspaceMode === 'studio' ? 'active' : ''} onClick={() => switchWorkspace('studio')}>3D Studio</button><button className={workspaceMode === 'viewmaster' ? 'active' : ''} onClick={() => switchWorkspace('viewmaster')}>View-Master Reel</button><button className={workspaceMode === 'polarized' ? 'active' : ''} onClick={() => switchWorkspace('polarized')}>Polarized Projection</button></nav><div className={`localBadge stage-${processingStage}`}><span className="statusDot" /> {visibleStageLabel}</div></div></header>
        <div hidden={workspaceMode !== 'studio'}><main className={`workspace ${sidebarCollapsed ? 'railCollapsed' : ''}`} style={workspaceStyle}><aside className="controlRail"><button className="collapseRail" onClick={() => setSidebarCollapsed(true)} title="Collapse source panel" aria-label="Collapse source panel">‹</button><div className="sourceModeSwitch" aria-label="Source type"><button className={studioInputMode === 'single' ? 'active' : ''} onClick={() => switchInputMode('single')}>Single image</button><button className={studioInputMode === 'pair' ? 'active' : ''} onClick={() => switchInputMode('pair')}>Stereo pair</button></div>{studioInputMode === 'single' ? <ImageUpload setIsDepthMapReadyStateLifter={setIsDepthMapReady} isChangeAllowed={isChangeAllowed} setIsChangeAllowed={setIsChangeAllowed} setProcessingStage={setProcessingStage} onSourceFile={setSingleSourceFile} incomingSourceFile={incomingSingleFile} onIncomingSourceConsumed={() => setIncomingSingleFile(null)} /> : <StereoPairUpload pair={stereoPair} onChange={(next) => { setStereoPair(next); setProcessingStage(next.left && next.right ? 'ready' : 'idle') }} />}<button className="sourceTransferAction" disabled={!currentStudioSource} onClick={() => sendToViewMaster()}>{studioInputMode === 'pair' ? 'Add current pair to View-Master' : 'Add current image to View-Master'}</button></aside><div className="railResizer" onMouseDown={beginResize} title="Drag to resize source panel" /><section className="stagePanel">{sidebarCollapsed && <button className="expandRail" onClick={() => setSidebarCollapsed(false)} title="Show source panel">› Source</button>}{studioSurface === 'printpage' ? <div className="embeddedTechniqueSurface"><button className="returnToTechniques" onClick={() => setStudioSurface('editor')}><UiIcon name="back" /> Back to 3D techniques</button><PrintPageBuilder setProcessingStage={setProcessingStage} incomingArtwork={printPageIncoming} onIncomingArtworkConsumed={() => setPrintPageIncoming(null)} /></div> : studioInputMode === 'pair' ? <StereoPairEditor pair={stereoPair} setProcessingStage={setProcessingStage} onSendToViewMaster={() => sendToViewMaster()} onPreparePrintPage={sendOutputToPrintPage} /> : studioSurface === 'phantogram' ? <div className="embeddedTechniqueSurface"><button className="returnToTechniques" onClick={() => setStudioSurface('editor')}><UiIcon name="back" /> Back to 3D techniques</button><PhantogramBuilder isDepthMapReady={isDepthMapReady} sourceFile={singleSourceFile} setProcessingStage={setProcessingStage} /></div> : studioSurface === 'colorreveal' ? <div className="embeddedTechniqueSurface"><button className="returnToTechniques" onClick={() => setStudioSurface('editor')}><UiIcon name="back" /> Back to 3D techniques</button><ColorRevealBuilder setProcessingStage={setProcessingStage} /></div> : studioSurface === 'layered' ? <div className="embeddedTechniqueSurface"><button className="returnToTechniques" onClick={() => setStudioSurface('editor')}><UiIcon name="back" /> Back to 3D techniques</button><LayeredCompositeBuilder isDepthMapReady={isDepthMapReady} setProcessingStage={setProcessingStage} /></div> : studioSurface === 'printcalibration' ? <div className="embeddedTechniqueSurface"><button className="returnToTechniques" onClick={() => setStudioSurface('editor')}><UiIcon name="back" /> Back to 3D techniques</button><PrintCalibrationBuilder setProcessingStage={setProcessingStage} /></div> : <AnaglyphEditor isDepthMapReady={isDepthMapReady} isChangeAllowed={isChangeAllowed} setIsChangeAllowed={setIsChangeAllowed} setProcessingStage={setProcessingStage} onOpenPhantogram={() => setStudioSurface('phantogram')} onOpenColorReveal={() => setStudioSurface('colorreveal')} onOpenLayered={() => setStudioSurface('layered')} onOpenPrintCalibration={() => setStudioSurface('printcalibration')} onOpenPrintPage={openPrintPage} onPreparePrintPage={sendOutputToPrintPage} />}</section></main></div>
