...
def main():
    ...
    yellow_blue = make_anaglyph(image, view, 'yellow-blue', 'gray', '#ffff00', '#0000ff', 90, 110)
    ...
    assert yellow_blue.shape == image.shape and np.max(yellow_blue) > 0
    assert custom_pair.shape == image.shape and np.max(custom_pair) > 0
    assert not np.array_equal(yellow_blue, custom_pair)
